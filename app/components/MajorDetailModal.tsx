    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xl my-8 animate-in fade-in zoom-in duration-150">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              <ul className="space-y-2">
                {jurusan.keahlian.map((k, i) => (
                  <li key={i} className="flex items-center gap-2 text-xs text-slate-700"><LuCircleCheckBig className="w-3.5 h-3.5 text-sky-600 shrink-0" /><span>{k}</span></li>
                ))}
              </ul>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {jurusan.prospek.map((p, i) => (
                <div key={i} className="p-2.5 rounded-lg bg-sky-100/60 border border-sky-100 text-center">
                  <span className="text-xs font-semibold text-sky-900 block">{p}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <a href="#spmb" onClick={onClose} className="px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-600 text-white text-xs font-semibold transition-colors">Daftar Jurusan Ini</a>
        </div>
      </div>
    </div>
  );
